...
const numberMatrixElements = 5100;
const min = 1;
const max = 10;
...
function startWorkers(threadCount) {
	return new Promise((resolve, reject) => {
		multiThreadWorkers(matrixA, matrixB, threadCount)
			.then((workers) => {
				...
				for (let i = 0; i < workers.length; i++) {
					workers[i].on('message', (event) => {
						workerResults[event.chunkIndex] = event.result;
						completed++;
					...
					workers[i].on('error', (error) => {
						console.error(
							`Ошибка в воркере при ${threadCount} потоках:`,
							error
						);
